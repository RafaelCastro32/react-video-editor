...
import { IDisplay, IMetadata } from "@designcombo/types";
...
class Caption extends Resizable {
	static type = "Caption";
	public text: string;
	public metadata?: CaptionsProps["metadata"];
	...
	constructor(props: CaptionsProps) {
		super(props);
		// Use groupColor from metadata if available, otherwise use default color
		this.fill = props.metadata?.groupColor || "#1B272C";
	...
	public drawTextIdentity(ctx: CanvasRenderingContext2D) {
		...
		// Build display text with group indicator if available
		let displayText = this.text;
		if (this.metadata?.groupName) {
			displayText = `[${this.metadata.groupName}] ${this.text}`;
		}
