...
const GeneratedAudios = () => {
	...
	const kokoroAudios = uploads.filter(
		(upload) =>
			upload.type === "audio" && upload.id?.startsWith("kokoro-audio-"),
	);
						...
						>
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs text-muted-foreground">
									{audio.duration
										? `${(audio.duration / 1000).toFixed(1)}s`
										: "Unknown duration"}
								</p>
							</div>
...
export const AiVoice = () => {
	...
	const handlePlayPause = (voiceId: string) => {
		...
		toast.info(
			"Preview not available for Kokoro voices. Generate audio to hear the voice.",
		);
	};
			...
			if (response.ok) {
				...
				if (queryParams?.gender && queryParams.gender !== "all") {
					filteredVoices = filteredVoices.filter(
						(v: KokoroVoice) => v.gender === queryParams.gender,
					);
				}
				...
				if (filteredVoices.length > 0 && !selectedVoice) {
					const defaultVoice =
						filteredVoices.find((v: KokoroVoice) => v.id === "pf_dora") ||
						filteredVoices[0];
				...
				toast.error(
					"Failed to load Kokoro voices. Make sure Python environment is set up.",
				);
			}
		} catch (error) {
